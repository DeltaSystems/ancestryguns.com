...
$details = sprintf( '<a href="%s" rel="nofollow" title="%s" class="details">%s</a>', get_permalink( $product->get_id() ), apply_filters( 'yit_details_button', __( 'Details', 'yit' ) ), apply_filters( 'yit_details_button', __( 'Details', 'yit' ) ) );
...
if ( $show_share ) {
	if ( isset( WC()->integrations->integrations['sharethis'] ) && WC()->integrations->integrations['sharethis']->publisher_id ) {
		...
		$share = sprintf( '<a href="%s" rel="nofollow" title="%s" class="share" id="%s" onclick="return false;">%s</a>', '#', __( 'Share', 'yit' ), "share_$product->get_id()", __( 'Share', 'yit' ) );
	} else {
...
elseif ( ! $product->is_in_stock() ) {
	...
	$out_of_stock = sprintf( '<a href="%s" class="%s" title="%s">%s</a>',$product->add_to_cart_url(), $out_of_stock_class, $label, $label );
	...
	$add_to_cart = apply_filters( 'woocommerce_loop_add_to_cart_link', // WPCS: XSS ok.
...
if ( ! empty( $add_to_cart ) || ! empty( $details ) ) : ?>
	<div class="product-actions-wrapper">
		<div class="product-actions">
			...
			if ( yit_get_option( 'shop-view-show-rating' ) && ! $is_wishlist ) {
				echo wc_get_rating_html( 'shop_loop' );
			}
			...
			if ( $product->is_purchasable() || $porduct_type == 'external' ) {
				echo $add_to_cart;
			}
